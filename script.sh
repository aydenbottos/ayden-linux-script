...
echo "Last Modified on July 16, 2022"
echo "Linux script"
...
apt-get install apt-transport-https dirmngr ufw tcpd lynis chkrootkit net-tools iptables libpam-cracklib apparmor apparmor-utils apparmor-profiles-extra clamav clamav-freshclam auditd audispd-plugins ecryptfs-utils cryptsetup aide unhide psad -y
...
then
	...
	find /home -regextype posix-extended -regex '.*\.(tiff|tif|rs|iml|gif|jpeg|exe|torrent|run|bat|jpg|jpe|png|rgb|xwd|xpm|ppm|pbm|pgm|pcx|ico|svg|svgz|pot|xml|pl)$' -delete
...
echo -e "auth [default=die] pam_faillock.so authfail\n auth sufficient pam_faillock.so authsucc\nauth requisite pam_faillock.so preauth" >> /etc/pam.d/common-auth
